...
public class Worker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            ...
            _logger.LogInformation("Logging into Discord using token: {Token}", _discordSettings.Token);
        ...
        finally { await _client.StopAsync(); }
    }
    ...
    private async Task SendNotificationsAsync()
    {
        ...
        {
            if (notification.NotifySunrise != null && notification.NextNotifySunriseUtc < currentDate) { await NotifySunriseAsync(notification, currentDate); }

            if (notification.NotifySunset != null && notification.NextNotifySunsetUtc < currentDate) { await NotifySunsetAsync(notification, currentDate); }
        }
    }
    ...
    private async Task SetNextNotificationTimesAsync()
    {
        ...
        foreach (var notification in notifications)
        {
            if (notification.NotifySunrise != null && notification.NextNotifySunriseUtc == null) { UpdateSunrise(notification, currentDate); }

            if (notification.NotifySunset != null && notification.NextNotifySunsetUtc == null) { UpdateSunset(notification, currentDate); }
    ...
    private void UpdateSunrise(Notification notification, DateTime currentDate)
    {
        ...
        {
            date = date.AddDays(1);
            calculation = new(notification.Latitude, notification.Longitude, date);
        }
